...
| 6 | Branch → commits → PR → canonical CI → exact-green-head merge | merge commit on `main`, required checks SUCCESS on exact head | PENDING |
| 7 | Release evaluation: docs-only ⇒ no userscript release | no release commit expected; Tampermonkey update NOT REQUIRED | PENDING |
